...
{	
	...
	// TEMPORARY FIX -- Feather disable GM2022
...
function TGMX_Tween(_script, _args, _tID)
{
		...
		{	 // REGULAR PROPERTY
			...
			if (is_numeric(_argLabel)) 
			{
				...
				case 64: // "@" Event Callback -- This makes sure that we use the right assigned target before actually adding the callbacks later in this function
					...
					_qCallbacks ??= ds_queue_create(); // NEWLY ADDED
					i += 1;
	...
	if (is_string(_t[TGMX_T_EASE])) // CONVERT EASE TYPE IF A STRING
	{
		...
		// THIS IS A TEMPORARY FIX FOR HTML5 ISSUE -- IS THIS NEEDED ANYMORE??
		//if (!is_struct(_t[TGMX_T_EASE])) { _t[@ TGMX_T_EASE] = method(undefined, method_get_index(_t[TGMX_T_EASE])); }
	}
	else
...
function TGMX_TweenPreprocess(_t) 
{
		...
		repeat(2)
		{
			...
			{	// REMOVE ANY SPACES IN STRING
				...
				{		
					...
					// PERFORM OPERATION -- Feather disable GM1010
	...
	if (!is_real(_t[TGMX_T_DURATION]))
	{
		...
		{	
			...
			var _absChangeSum = 0;
			var _count = _struct[$ "use"] ?? array_length(_data) div 4; // NEWLY ADDED
			...
			{
				_t[@ TGMX_T_DURATION] = _absChangeSum/_struct[$ "rate"];
			}
			...
			if (!is_undefined(_struct[$ "weight"]))
			{
				...
				_t[@ TGMX_T_DURATION] = _output / _struct.weight;
			}
			else
			...
			{	
				...
				_t[@ TGMX_T_DURATION] = _max / _struct.max;
			}
			else
			if (!is_undefined(_struct[$ "min"]))
			{
				...
				_t[@ TGMX_T_DURATION] = _min / _struct.min;
			}
			else // Divide Duration by Property Count "split"
			{
				_t[@ TGMX_T_DURATION] = _absChangeSum/_struct[$ "split"]/_count;
			}
		}
	}
	...
	{	
		...
		else // Specific Target
		{
			...
			repeat(ds_list_size(_tweens))
			{
				...
				if (TGMX_TargetExists(_target))
				{
					if (is_struct(_target)) // STRUCT TARGET -- Feather disable GM1009
					{
					...
					else // INSTANCE | OBJECT | CHILD -- Feather disable GM1041 Feather disable GM1009
					{
						if (_target == _selectionData.id || _target.object_index == _selectionData || object_is_ancestor(_target.object_index, _selectionData))
						{
							_args[0] = _t;
